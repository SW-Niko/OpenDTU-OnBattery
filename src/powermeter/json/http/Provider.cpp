...
void Provider::pollingLoop()
{
    ...
    uint32_t minPollWait = _cfg.PollingIntervalMs / 10; // minimum poll wait time in milliseconds
    uint32_t sleepMs = 0;

    while (!_stopPolling) {

        // determine the sleep duration based on pause or polling interval
        if (_pausePolling) {
            sleepMs = _pauseDuration;
            usePauseInterval = true;
            _trigger = false; // reset the trigger when entering pause
            _lastPoll = millis(); // to do logging correctly
        } else {
            // calculate the sleep duration and ensure a minimum wait time
            uint32_t elapsedMillis = millis() - _lastPoll;
            sleepMs = ((_lastPoll > 0) && (elapsedMillis < intervalMillis)) ? (intervalMillis - elapsedMillis) : minPollWait;
            sleepMs = std::max(sleepMs, minPollWait);
        ...
        _trigger = false; // reset the trigger after waking up

        // skip polling if we are stopping or if we are in a pause without using the pause interval
        if (_stopPolling || (_pausePolling && !usePauseInterval)) {
            continue;
        }
...
{
    {
        ...
        if (duration == 0) {
            ...
            _pausePolling = true;
            duration = std::clamp(duration, 100u, 15u * 1000u); // range 100ms to 15s
            _pauseDuration = duration;
            DTU_LOGD("pause for a duration of: %ums [min: 100ms, max: 15s]", _pauseDuration);
        }
        _trigger = true;
    }
